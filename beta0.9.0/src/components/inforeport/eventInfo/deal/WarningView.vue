  ...
  <div class="instruction" @click="hiddenShell" :class="mini">
    ...
    <div class="mainScroll" @scroll="showScroll" ref="mainScroll" :class="scrollerHeight">
      ...
      <div class="main_panel" :class="{'instruct':showInstr}" v-show="showInstr">
        <instruct-import @on-add-click="showComment=true"
                         :eventId="id"
                         ref="importPanel"
      ...
      <box gap="10px 10px">
        <div class="popup0">
          <group>
            ...
            <!-- <leader-selector leaderType='2' v-model="selectInstrLeadList" label="选择领导" maxLength="3"></leader-selector> -->
          </group>
          <group>
...
<script>
  ...
  import WarnItem from './../../commoncompents/WarnItem.vue'
      ...
      next(
        (vm) => {
          if (from.name === 'eventTypeSelect') {
            ...
            vm.selectInstrLeadList.push(eventType.code)
...
//            页面滚动高度
            ...
            if (from.name === 'TransTabs' || from.name === 'InstructInfo') {
              console.log('我从详情进来的额')
              vm.isWatch = false
            }
...
//      进入编辑
      infoEdit() {
        this.$router.push(`/report/editWaringInfo/${this.id}`)
      },
//    控制审核页面回复内容的设置
      setList() {
      ...
      setListPanel() {
        this.$refs.setList.setListPanel()
      },
//        进入编辑页面
      enterEdit(e) {
...
//        审核
        ...
        await vm.$http.get(URLA).then((res) => {
...
//      拟办意见
      async instructPanel() {
        ...
        await vm.$http.get(URLP).then((res) => {
...
//        转办督办 5
        ...
        await vm.$http.get(URLT).then((res) => {
...
//      领导批示
      async loadDeal(id) {
        ...
        await vm.$http.get(URLD).then((res) => {
...
//                  刷新转办列表
                  vm.transLoad()
                  vm.$refs.timelinePanel.init(this.id)
                } else {
                  vm.$vux.toast.text(res.body.message, 'middle')
                  // alert(res.body.message)
                }
        ...
        await this.$http.get(URL).then(function (response) {
          if (response.body.code === 200) {
            ...
            if (this.eventBase.attachExist) {
              that.attachs = JSON.parse(that.eventBase.attachs)
            }
